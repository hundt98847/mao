...
// TODO(martint): check out @PLT and @GOT variants
      ...
      if (instruction_->types[i].bitfield.disp8 ||
        ...
        if (instruction_->op[i].disps->X_op == O_symbol) {
          fprintf(out, "%s",
                  S_GET_NAME(instruction_->op[i].disps->X_add_symbol) );
          // PLT?
          if (instruction_->reloc[i] == BFD_RELOC_X86_64_PLT32) {
            fprintf(out, "%s", "@PLT");
          }
        }
        if (instruction_->op[i].disps->X_op == O_subtract &&
            instruction_->op[i].disps->X_add_symbol != NULL) {
          fprintf(out, "%s",
                  S_GET_NAME(instruction_->op[i].disps->X_add_symbol) );
          // GOT?
          if (instruction_->reloc[i] == BFD_RELOC_32_PCREL) {
            fprintf(out, "%s", "@GOTPCREL");
          }
        }
        if (instruction_->op[i].disps->X_op == O_symbol &&
